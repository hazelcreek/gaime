"""
Classic engine models - Pydantic models for the classic game engine.

These models are specific to the classic engine's single-LLM architecture.
"""
...
class StateChanges(BaseModel):
    ...
    )  # World-defined flags (set by interactions)
...
class GameState(BaseModel):
    """Current game session state for the classic engine"""
    ...
    npc_locations: dict[str, str] = Field(
        default_factory=dict
    )  # Current NPC locations (npc_id -> location_id)
    status: str = "playing"  # "playing", "won", "lost"
    narrative_memory: NarrativeMemory = Field(
        default_factory=NarrativeMemory
