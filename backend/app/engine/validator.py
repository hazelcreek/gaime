    ...
    def _collect_flags(self):
            ...
            if location.interactions:
                for int_id, interaction in location.interactions.items():
                    if interaction.sets_flag:
                        self._record_flag_set(interaction.sets_flag, f"location:{loc_id}/interaction:{int_id}")
        ...
        for item_id, item in self.world_data.items.items():
            if item.use_actions:
                for action_id, action in item.use_actions.items():
                    if action.sets_flag:
                        self._record_flag_set(action.sets_flag, f"item:{item_id}/action:{action_id}")
        
        # Flags CHECKED by location requires
        for loc_id, location in self.world_data.locations.items():
            if location.requires and location.requires.flag:
                self._record_flag_checked(location.requires.flag, f"location:{loc_id}/requires")
        ...
        for item_id, item in self.world_data.items.items():
            if item.find_condition:
                ...
                if required_flag:
                    self._record_flag_checked(required_flag, f"item:{item_id}/find_condition")
            ...
            if npc.appears_when:
                for condition in npc.appears_when:
                    if condition.condition == "has_flag":
                        self._record_flag_checked(str(condition.value), f"npc:{npc_id}/appears_when")
        
        # Flags CHECKED by NPC location_changes
        for npc_id, npc in self.world_data.npcs.items():
            for change in npc.location_changes:
                if change.when_flag:
                    self._record_flag_checked(change.when_flag, f"npc:{npc_id}/location_changes")
        
        # Flags CHECKED by victory condition
        if self.world_data.world.victory and self.world_data.world.victory.flag:
            self._record_flag_checked(self.world_data.world.victory.flag, "world/victory")
...
def validate_world(world_id: str, worlds_dir: str | Path | None = None) -> ValidationResult:
    """
