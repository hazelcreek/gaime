...
async def get_completion(
    ...
    logger.debug(f"Messages: {len(messages)} messages, response_format={response_format}")
    ...
    try:
        ...
        if finish_reason == "length":
            logger.warning(f"Response TRUNCATED due to max_tokens limit ({max_tokens}). Consider increasing max_tokens.")
...
def parse_json_response(response: str | None, strict: bool = False) -> dict:
    ...
    try:
        ...
        narrative_match = re.search(r'"narrative"\s*:\s*"((?:[^"\\]|\\.)*)(?:"|$)', cleaned)
        if narrative_match:
